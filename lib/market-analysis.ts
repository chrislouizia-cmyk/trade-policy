...
export type ConfidenceBreakdown={mandatoryConfirmed:string[];mandatoryMissing:string[];optionalConfirmed:string[];contradicted:string[];unsupported:string[];manual:string[]};
...
function scoreConfidence(strategy:StrategyProfile,evidence:Record<string,EvidenceAssessment>,aligned:boolean){
  ...
  const enabled=allEnabled.filter(r=>r.evaluationMode!=='MANUAL');
  ...
  return {liveAnalysisConfidence,components,breakdown:{mandatoryConfirmed:confirmedMandatory,mandatoryMissing:missingMandatory,optionalConfirmed:confirmedOptional,contradicted,unsupported,manual},relevant};
}
...
export function buildLiveAnalysis(instrument:Instrument,strategy:StrategyProfile,series:Record<string,Candle[]>,provider:string,normalizedProviderSymbol=instrument):LiveMarketAnalysis{
  ...
  const warnings=[...scored.breakdown.contradicted];if(scored.breakdown.unsupported.length)warnings.push(`Unreachable automatic evidence: ${scored.breakdown.unsupported.join(', ')}.`);if(scored.breakdown.manual.length)warnings.push(`Manual confirmation required: ${scored.breakdown.manual.join(', ')}.`);if(!direction)warnings.push('No directional setup is currently supported by the configured evidence.');
  ...
  const layerAnalysis=layers.map(layer=>{const layerRules=(strategy.rules??[]).filter(rule=>rule.enabled&&rule.timeframeRole===layer.role&&rule.evaluationMode!=='MANUAL').map(rule=>normalizeEvidenceId(rule.ruleKey)).filter(id=>(DETECTOR_EVIDENCE_IDS as readonly string[]).includes(id));const confirmedEvidence=layerRules.filter(id=>evidence[id]?.value);return {...layer,bias:timeframes[layer.timeframe].bias,confirmedEvidence,missingEvidence:layerRules.filter(id=>!confirmedEvidence.includes(id)),confidence:layerRules.length?Math.round(confirmedEvidence.length/layerRules.length*100):null}});
