...
export type ConditionEvidence={id:string;ruleId:string;label:string;status:RuntimeStatus;required:boolean;evaluationType:TradingDnaEvaluationType;operator:TradingDnaOperator;actual:unknown;expected:unknown[];reason:string;groupPath:string[]};
...
export function evaluateTradingDnaRuntime(rules:StrategyProfile['rules'],context:TradingDnaRuntimeContext,now=()=>new Date().toISOString()):TradingDnaEvidenceReport{
  ...
  function evaluateCondition(condition:ComposerCondition,path:string[]):RuntimeStatus{
    ...
    conditions.push({id:condition.id,ruleId:condition.ruleId,label:summarizeComposerCondition(condition),status,required:condition.legacyRule?.mandatory??true,evaluationType,operator:condition.operator,actual:fact.value,expected:condition.operands,reason,groupPath:path});return status;
  }
