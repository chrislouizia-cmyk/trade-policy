...
export function buildHistoricalDecisionSnapshot({userId,analysis,strategy,input,result,explanation,reportId=randomUUID()}:{userId:string;analysis:ChartAnalysis;strategy:StrategyProfile;input:TradeInput;result:TradeResult;explanation:DecisionExplanationSummary;reportId?:string}):HistoricalDecisionReportSnapshot{
  ...
  const base:HistoricalDecisionReportSnapshot={
    ...
    finalRiskCheck:{status:result.verdict==='AUTHORIZED'?'PASSED':result.verdict==='WAIT'?'INCOMPLETE':'FAILED',entryPrice:input.entry,stopPrice:input.stopLoss,targetPrice:input.takeProfit,riskReward:result.rr,riskPercent:input.riskPercent,blockingReasons:[...result.vetoes],overrideEligible:result.overrideEligible===true,overrideBlockers:result.overrideBlockers??[],vetoes:(result.finalRiskBlocks??[]).map(block=>({id:block.id,label:block.name,overrideable:block.overrideable,actual:block.actual,limit:block.required,source:block.source,reason:block.reason}))},
