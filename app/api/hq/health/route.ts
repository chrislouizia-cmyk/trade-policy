...
export async function GET(){
  ...
  else try{const admin=createAdminClient(),day=new Date();day.setUTCHours(0,0,0,0);const rollingMinuteStart=new Date(Date.now()-60_000).toISOString();const [minuteEventResult,dayResult,eventResult]=await Promise.all([admin.from('provider_credit_events').select('credits').eq('provider','twelvedata').eq('allowed',true).gte('created_at',rollingMinuteStart),admin.from('provider_credit_windows').select('credits_used').eq('provider','twelvedata').eq('window_kind','DAY').eq('window_start',day.toISOString()).maybeSingle(),admin.from('provider_credit_events').select('operation,credits').eq('provider','twelvedata').eq('allowed',true).gte('created_at',day.toISOString()).order('created_at',{ascending:false}).limit(250)]);if(minuteEventResult.error||dayResult.error||eventResult.error)throw minuteEventResult.error??dayResult.error??eventResult.error;const minuteUsed=(minuteEventResult.data??[]).reduce((sum,row)=>sum+Number(row.credits),0),dailyUsed=Number(dayResult.data?.credits_used??0),totals=new Map<string,number>();for(const row of eventResult.data??[])totals.set(row.operation,(totals.get(row.operation)??0)+Number(row.credits));const recentOperations=[...totals].map(([operation,credits])=>({operation,credits})).sort((a,b)=>b.credits-a.credits).slice(0,5);services.twelveData={status:dailyUsed>=800?'degraded':'operational',message:`Credit coordinator active · ${minuteUsed}/8 rolling minute · ${dailyUsed}/800 today`,budget:{minuteUsed,minuteLimit:8,dailyUsed,dailyLimit:800,dailyResetsAt:new Date(day.getTime()+86_400_000).toISOString(),recentOperations}}}catch(error){services.twelveData={status:'degraded',message:'Provider configured; credit telemetry unavailable'}}
