...
import {isTwelveDataRateLimit} from '@/lib/market-data';
...
export async function GET(){
  ...
  else try{const check=await timed(async signal=>{const url=new URL('https://api.twelvedata.com/price');url.searchParams.set('symbol','AAPL');url.searchParams.set('apikey',process.env.TWELVE_DATA_API_KEY!);const response=await fetch(url,{signal,cache:'no-store'});const body=await response.json();const message=String(body?.message??'Provider rejected probe');if(isTwelveDataRateLimit(response.status,message))return {rateLimited:true};if(!response.ok||body.status==='error'||!Number.isFinite(Number(body.price)))throw new Error(message);return {rateLimited:false}});services.twelveData=check.value.rateLimited?{status:'degraded',latencyMs:check.latencyMs,message:'Provider minute limit reached; retry after the current minute'}:success(check.latencyMs)}catch(error){services.twelveData=unavailable(error)}
