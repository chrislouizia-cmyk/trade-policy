...
export async function GET(request:Request){
  ...
  const {data:canViewTrading}=await supabase.rpc('has_staff_permission',{p_permission:'customers.view_trading'});
  const url=new URL(request.url),q=(url.searchParams.get('q')||'').slice(0,120),requestedSort=sorts.has(url.searchParams.get('sort')||'')?url.searchParams.get('sort')!:'last_activity',sort=!canViewTrading&&['account_count','analysis_count'].includes(requestedSort)?'last_activity':requestedSort,direction=url.searchParams.get('direction')==='asc'?'asc':'desc';
  ...
  const metadataColumns=[{header:'Customer ID',key:'customer_id',width:38},{header:'Name',key:'display_name',width:24},{header:'Email',key:'email',width:32},{header:'Plan',key:'plan',width:14},{header:'Status',key:'subscription_status',width:18},{header:'Created At',key:'created_at',width:24},{header:'Last Activity',key:'last_activity_at',width:24}];
  const tradingColumns=[{header:'Active Strategy',key:'active_strategy',width:24},{header:'Trading Accounts',key:'account_count',width:18},{header:'Analyses',key:'analysis_count',width:12},{header:'Open Trades',key:'open_trades',width:14},{header:'Closed Trades',key:'closed_trades',width:14}];
  sheet.columns=canViewTrading?[...metadataColumns,...tradingColumns]:metadataColumns;
  sheet.addRows(rows);sheet.getRow(1).font={bold:true};sheet.autoFilter={from:'A1',to:`${canViewTrading?'L':'G'}1`};
