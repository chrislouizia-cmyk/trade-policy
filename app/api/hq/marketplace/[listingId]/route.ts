...
export async function GET(_request:Request,{params}:{params:Promise<{listingId:string}>}){
  try{
    ...
    if(release){const {error:metricsRefreshError}=await admin.rpc('refresh_marketplace_release_verified_metrics',{p_marketplace_release_id:release.id,p_source_strategy_id:release.source_strategy_id,p_source_strategy_revision_id:release.source_strategy_revision_id});if(metricsRefreshError)throw metricsRefreshError;}
    ...
    if(!release)return NextResponse.json({error:'Marketplace release not found.'},{status:404});
    const evidence=await buildMarketplaceEvidence(admin,release.source_strategy_id,release.source_strategy_revision_id);
