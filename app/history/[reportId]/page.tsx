...
export default async function HistoricalReportPage({params}:{params:Promise<{reportId:string}>}){
  ...
  return <main className="container historical-report-page"><AppHeader eyebrow="TRADE POLICE / HISTORY" displayName={displayName} description="Saved Decision Report" userId={user.id}/><nav aria-label="History breadcrumb"><a href="/history?view=decisions">← All saved reports</a></nav><HistoricalDecisionReport snapshot={parsed.snapshot} ai={ai}/></main>;
}
