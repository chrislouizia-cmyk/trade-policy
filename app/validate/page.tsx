...
export default async function ValidatePage() {
  ...
  try {
    ...
    return <main className="container"><AppHeader eyebrow="TRADE POLICE / VALIDATION DESK" displayName={displayName} description="Request authorization before executing a trade." userId={user.id} /><div className="card empty-state"><h2>{error instanceof NoActiveStrategyError?'No active strategy':'Active strategy needs configuration'}</h2><p className="muted">{error.message}</p><a className="button-link primary" href="/profile">Build or configure strategy</a></div></main>;
  }
  return <main className="container"><AppHeader eyebrow="TRADE POLICE / VALIDATION DESK" displayName={displayName} description="Request authorization before executing a trade." userId={user.id} /><TradeValidator userId={user.id} displayName={displayName} initialStrategy={strategy} /></main>;
}
