...
export default async function AnalyticsPage(){
 ...
 if(tradeResult.error){
  ...
  return <main className="container"><AppHeader eyebrow="TRADE POLICE / ANALYTICS" displayName={displayName} description="Turn disciplined execution into evidence." userId={user.id}/><section className="card error-state"><h1>Analytics could not be loaded</h1><p>Your closed trades remain recorded. Please refresh and try again.</p></section></main>;
 }
 ...
 const simulationTrades=allTrades.filter((x:any)=>isTradeLifecycleSimulationRecord(x)).map((x:any)=>{
  ...
  const simulationMode=(strategy_snapshot.simulationMode ?? strategy_snapshot.testSource ?? strategy_snapshot.internalTestMode ?? 'INTERNAL_LIFECYCLE_SMOKE_TEST');
