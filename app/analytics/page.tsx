...
export default async function AnalyticsPage(){
 ...
 const [{data:account},{data=[]}]=await Promise.all([
  ...
  s.from('trade_records').select('id,result_r,realized_pnl,outcome,taken_against_verdict,strategy_name_at_entry,instrument,initial_analysis,opened_at,closed_at,direction,risk_percent,initial_rr,setup_type,override_reason,strategy_snapshot,simulation_mode,source,source_report_id,activation_mode').eq('user_id',user.id).eq('status','CLOSED').order('closed_at')
 ]);
 ...
 const liveTrades=allTrades.filter((x:any)=>!isTradeLifecycleSimulationRecord(x)).map((x:any)=>({
  ...
  simulationMode:x.simulation_mode ?? null,
 ...
 return <main className="container"><AppHeader eyebrow="TRADE POLICE / ANALYTICS" displayName={displayName} description="Turn disciplined execution into evidence." userId={user.id}/>{simulationTrades.length>0?<section className="card analytics-verification-card" style={{marginBottom:'1.5rem'}}><div className="section-title"><div><span className="eyebrow">SIMULATION / INTERNAL TEST</span><h2>Verification-only records</h2><p className="muted">These trades never count toward premium live-performance analytics.</p></div></div><div className="historical-card-list">{simulationTrades.map(trade=><article className="historical-card" key={trade.id}><div><time dateTime={trade.closedAt}>{new Date(trade.closedAt).toLocaleString()}</time><strong>{trade.instrument}</strong><span>{trade.strategy}</span></div><div><span className="badge blocked">{trade.label}</span><p>{trade.outcome}</p></div><dl><div><dt>R</dt><dd>{trade.r.toFixed(2)}R</dd></div><div><dt>Mode</dt><dd>{trade.simulationMode}</dd></div></dl></article>)}</div></section>:null}<AnalyticsDashboard account={{name:account?.name??'Active account',currency:account?.currency??'USD',startingBalance:Number(account?.initial_balance??0),currentBalance:Number(account?.current_balance??0)}} trades={liveTrades}/></main>
}
