...
export default async function LandingPage() {
  const [headerStore,cookieStore,locale]=await Promise.all([headers(),cookies(),getRequestLocale()]);
  ...
  const hasSessionHint=authCookieNames.some(name=>/(?:auth-token|refresh-token)(?:\.\d+)?$/i.test(name));
  ...
  return <main className={styles.page}>
    <nav className={styles.nav} aria-label={c.navigation}>
      ...
      <div className={styles.navActions}><Link href={accountHref}>{c.signIn}</Link><Link className={styles.navCta} href={signupHref}>{c.startFree}</Link></div>
    </nav>

    <section className={styles.hero}>
      <div className={styles.heroCopy}><p className={styles.kicker}><span/>{c.kicker}</p><h1>{c.heroTitle}<em>{c.heroAccent}</em></h1><p className={styles.heroLead}>{c.heroLead}</p><div className={styles.actions}><Link className={styles.primaryCta} href={signupHref}>{c.checkSetup}<span>→</span></Link><a className={styles.secondaryCta} href="#system">{c.seeSystem}</a></div><p className={styles.micro}>{c.disclaimer}</p></div>
    ...
    <section className={styles.finalCta}><div className={styles.finalMark}><Image src="/brand/trade-police-mark.png" alt="" width={96} height={96}/></div><p className={styles.kicker}>{c.finalKicker}</p><h2>{c.finalTitle}</h2><p>{c.finalCopy}</p><Link className={styles.primaryCta} href={signupHref}>{c.startFree}<span>→</span></Link></section>
