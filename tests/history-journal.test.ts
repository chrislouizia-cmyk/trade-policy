...
test('History page reads canonical trades and decisions and exposes truthful journal controls', () => {
  ...
  assert.match(page, /c\.allActivity/);
  assert.match(page, /c\.openTrades/);
  assert.match(page, /c\.realizedR/);
  assert.match(page, /c\.integrity/);
  assert.match(page, /c\.opened.*formatDateTime\(item\.openedAt, locale\)/);
  assert.match(page, /c\.closedAt.*formatDateTime\(item\.closedAt, locale\)/);
  assert.match(page, /history-filter-disclosure/);
  assert.match(page, /history-timeline-marker/);
  assert.match(page, /c\.tradeDetails/);
