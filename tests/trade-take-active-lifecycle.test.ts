...
test('activation rpc enforces real database idempotency and removes the fake idempotency parameter',()=>{
  ...
  assert.doesNotMatch(route,/p_idempotency_key/);
  assert.match(route,/create unique index if not exists active_trades_user_source_decision_unique/);
  assert.match(route,/where source_decision_id is not null/);
  assert.match(route,/The API route authenticates the caller and passes the validated user_id to the server-owned activation boundary/);
  assert.doesNotMatch(route,/auth\.uid\(\) is null or p_user_id <> auth\.uid\(\)/);
});
...
test('activation rpc is not exposed to authenticated clients',()=>{
  assert.match(route,/revoke all on function public\.activate_trade_atomically_v1\(/);
  assert.match(route,/grant execute on function public\.activate_trade_atomically_v1\(/);
  assert.match(route,/to service_role/);
  assert.doesNotMatch(route,/to authenticated/);
});
