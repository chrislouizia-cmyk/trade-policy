...
function actionLabel(explanation:DecisionExplanationSummary|null, authoritativeVerdict?:string|null):string{
  ...
  if(verdict==='READY')return 'Run final risk check';
  if(verdict==='WAIT')return 'Complete required confirmations';
  if(verdict==='BLOCKED')return 'Review blocking conditions';
...
export default function DecisionHero({explanation,narrative,analyzing,authoritativeVerdict,onPrimaryAction,onViewReport,reportButtonRef,showReportButton=true}:Props){
  ...
  return <section className={`card decision-hero decision-explanation-hero state-${displayVerdict.toLowerCase()}`} aria-labelledby="decision-hero-title" aria-live="polite">
    ...
    <div className="decision-hero-actions"><button className="primary" type="button" onClick={onPrimaryAction}>{actionLabel(explanation, displayVerdict)}</button>{showReportButton?<button ref={reportButtonRef} type="button" className="decision-hero-report-button" onClick={onViewReport}><span aria-hidden="true">View full Decision Report</span><span className="sr-only">View Decision Report</span></button>:null}</div>
