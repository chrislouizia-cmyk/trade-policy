...
export default function BillingActions({mode,plan='PRO',interval:controlledInterval,onIntervalChange}:{mode:'checkout'|'portal';plan?:BillingPlan;interval?:BillingInterval;onIntervalChange?: (value: BillingInterval) => void}){
  ...
  const [interval,setInterval]=useState<BillingInterval>(controlledInterval ?? 'monthly');

  useEffect(()=>{if(controlledInterval!==undefined){setInterval(controlledInterval);}},[controlledInterval]);
  ...
  if(mode==='checkout'){const selectedInterval = controlledInterval ?? interval; const applyInterval=(next:BillingInterval)=>{setInterval(next);onIntervalChange?.(next);}; return <div className="billing-action"><div className="segmented-control" aria-label={`${plan} billing interval`}><button type="button" className={selectedInterval==='monthly'?'secondary selected':'secondary'} onClick={()=>applyInterval('monthly')}>Monthly</button><button type="button" className={selectedInterval==='annual'?'secondary selected':'secondary'} onClick={()=>applyInterval('annual')}>Annual</button></div><button className="primary" type="button" disabled={busy} onClick={act}>{busy?'Opening…':`Upgrade to ${plan}`}</button>{error&&<p className="error">{error}</p>}</div>;}
