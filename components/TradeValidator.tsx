...
export default function TradeValidator({userId,displayName,initialStrategy}:{userId:string;displayName:string;initialStrategy:StrategyProfile}) {
  ...
  async function saveTakenTrade(againstVerdict:boolean){
    ...
    try{
      ...
      setTradeActionMode(null);setTradeActionContext({againstVerdict:false,reason:null});
    ...
    {tradeActionMode&&<div className="reasoning-modal-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget){setTradeActionMode(null);setTradeActionContext({againstVerdict:false,reason:null});}}}>
      <section className="reasoning-modal" role="dialog" aria-modal="true" aria-labelledby="trade-action-title">
        ...
        <div className="reasoning-modal-body">
          ...
          {tradeActionMode==='ACTIVATE'&&<div className="reasoning-section"><h3>Take trade confirmation</h3><div className="trade-confirmation-summary"><p><strong>Account</strong> {selectedAccount?.name ?? 'Manual'}</p><p><strong>Instrument</strong> {getFieldValue('instrument')}</p><p><strong>Direction</strong> {getFieldValue('direction')}</p><p><strong>Entry</strong> {getFieldValue('entry')}</p><p><strong>Stop loss</strong> {getFieldValue('stopLoss')}</p><p><strong>Take profit</strong> {getFieldValue('takeProfit')}</p><p><strong>Risk/reward</strong> {result?.rr ? `1:${result.rr}` : '—'}</p><p><strong>Source</strong> {result?.reportSourceId ? `Decision ${result.reportSourceId}` : 'Pending'}</p></div><label>Why are you taking this trade{result?.verdict!=='AUTHORIZED'?' against the current verdict':''}?<textarea value={tradeActionContext.reason ?? ''} onChange={(event)=>setTradeActionContext(current=>({...current,reason:event.target.value}))} rows={4} placeholder={result?.verdict!=='AUTHORIZED'?'Describe the reason for taking it despite the current verdict.':'Describe your rationale for taking this trade.'}/></label><div className="discipline-action-row"><button type="button" onClick={()=>void saveTakenTrade(result?.verdict!=='AUTHORIZED' && tradeActionContext.reason?.trim()!=='' ? true : false)} disabled={savingTrade}>Confirm take trade</button><button type="button" onClick={()=>{setTradeActionMode(null);setTradeActionContext({againstVerdict:false,reason:null});}} disabled={savingTrade}>Cancel</button></div></div>}
