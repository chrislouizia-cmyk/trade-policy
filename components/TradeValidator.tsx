...
export default function TradeValidator({userId,displayName,initialStrategy}:{userId:string;displayName:string;initialStrategy:StrategyProfile}) {
  ...
  const [tradeActionMode,setTradeActionMode]=useState<'ACTIVATE'|'OVERRIDE'|'MISSED'|null>(null);
  ...
  async function saveTakenTrade(mode:'ACTIVATE'|'OVERRIDE'){
    ...
    try{
      ...
      setTradeActionMode(null);setTradeActionContext({againstVerdict:false,reason:null,confirmed:false});
  ...
  async function markTradeMissed(){
    ...
    finally { setSavingTrade(false); setTradeActionMode(null); setTradeActionContext({againstVerdict:false,reason:null,confirmed:false}); }
  }
