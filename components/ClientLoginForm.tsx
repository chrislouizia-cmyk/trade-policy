...
export default function ClientLoginForm({ next }: { next: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (staffRole) {
      ...
      setMessage('This email belongs to a Trade Police employee. Use the Headquarters login instead.');
      setLoading(false);
      return;
    }
  ...
  return (
    <form className="auth-form" onSubmit={submit}>
      ...
      <Link className="portal-switch" href="/hq/login">Employee? Open Headquarters login</Link>
    </form>
  );
}
