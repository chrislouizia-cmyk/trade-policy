-- Align the legacy atomic activation RPC with the real trade_records schema.
-- Strategy and lifecycle metadata remain on active_trades; trade_records stores
-- the frozen strategy payload in its existing rule_snapshot column.
