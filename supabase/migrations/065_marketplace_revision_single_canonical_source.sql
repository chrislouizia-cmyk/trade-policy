...
begin
  ...
  if p_canonical_strategy_text is null or btrim(p_canonical_strategy_text) = '' then
    raise exception 'Canonical strategy payload is required';
  end if;
  ...
  v_current_strategy := public.marketplace_normalize_strategy_profile_for_revision(v_source_profile.id);
  v_canonical_payload := p_canonical_strategy_text::jsonb;

  if jsonb_typeof(v_canonical_payload) <> 'object' then
  ...
  if not (v_canonical_payload ? 'strategy') then
    raise exception 'Strategy revision changed while preparing the internal test release. Reload the strategy before creating the release.';
  end if;

  v_normalized_strategy := v_canonical_payload -> 'strategy';

  if v_normalized_strategy <> v_current_strategy then
    raise exception 'Strategy revision changed while preparing the internal test release. Reload the strategy before creating the release.';
  end if;

  v_engine_version := coalesce(v_source_profile.engine_version, 1);
  v_expected_hash := encode(
    extensions.digest(
      convert_to(p_canonical_strategy_text, 'UTF8'),
      'sha256'
    ),
    'hex'
  );

  v_expected_source_revision_id := v_engine_version || ':' || v_expected_hash;
