...
begin
 ...
 -- The release snapshot is server-only. Clone only inside this controlled RPC.
 ...
 -- A recipient receives an independent copy of every persisted child row. The
 -- immutable release and its creator-owned source are never updated by install.
