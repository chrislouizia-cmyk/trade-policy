create or replace function public.marketplace_canonicalize_jsonb(input jsonb)
returns jsonb
language plpgsql
immutable
...
begin
  ...
  if jsonb_typeof(input) = 'object' then
    select jsonb_object_agg(key, value)
      into output
      from (
        select key, public.marketplace_canonicalize_jsonb(value) as value
  ...
  if jsonb_typeof(input) = 'array' then
    select jsonb_agg(public.marketplace_canonicalize_jsonb(elem))
      into output
      from (
        select elem
        from jsonb_array_elements(input) with ordinality as a(elem, ord)
        order by ord
      ) ordered;
    return coalesce(output, '[]'::jsonb);
  end if;
...
begin
  ...
  v_expected_source_revision_id := format(
    ...
    encode(pgcrypto.digest(public.marketplace_canonicalize_jsonb(v_canonical_strategy)::text, 'sha256'), 'hex')
  );
