...
begin
  if p_user_id is null then
    raise exception 'User context is required for closure.';
  end if;

  if p_trade_id is null then
    raise exception 'A trade ID is required for closure.';
  end if;
  ...
  if not found then
    raise exception 'The active trade could not be found for this user.';
  end if;

  if v_trade.status = 'CLOSED' then
    return jsonb_build_object(
      'trade_id', v_trade.id,
      'trade_record_id', v_trade.trade_record_id,
      'status', 'CLOSED',
      'already_closed', true,
      'outcome', v_trade.outcome,
      'close_price', v_trade.close_price,
      'realized_pnl', v_trade.realized_pnl,
      'event_type', 'TRADE_CLOSED'
    );
  end if;

  v_risk_distance := abs(v_trade.entry - v_trade.stop_loss);
  if v_risk_distance is null or v_risk_distance = 0 then
    raise exception 'The trade risk distance is invalid.';
  end if;

  v_result_r := case
    when v_trade.direction = 'BUY' then (p_close_price - v_trade.entry) / v_risk_distance
    else (v_trade.entry - p_close_price) / v_risk_distance
  end;

  v_realized_pnl := (coalesce(v_trade.risk_amount, 0) * v_result_r) - greatest(coalesce(p_fees, 0), 0);
  v_close_outcome := case
    when v_result_r > 0.05 then 'WIN'
    when v_result_r < -0.05 then 'LOSS'
    else 'BREAKEVEN'
  end;

  if p_outcome is not null and lower(trim(p_outcome)) not in ('win', 'loss', 'breakeven') then
    raise exception 'Invalid close outcome supplied.';
  end if;
  ...
  if v_trade.trade_record_id is not null then
    ...
    set
      ...
      closed_at = v_now,
      updated_at = v_now
    where id = v_trade.trade_record_id
      and user_id = p_user_id;
  end if;
