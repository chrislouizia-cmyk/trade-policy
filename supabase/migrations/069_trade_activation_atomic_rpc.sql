-- Trade Police v21: atomic activation boundary
-- This RPC owns the activation boundary so a successful daily-limit slot is created only once,
-- and failed activations leave no executed row, no active trade, and no audit event behind.
...
begin
  ...
  if p_source_decision_id is not null and not exists (
  ...
  if p_source_report_id is not null and not exists (
    ...
    raise exception 'The originating decision report could not be verified.';
  end if;

  if p_source_decision_id is not null and p_source_report_id is not null and not exists (
    select 1
